...
import { 
  Upload, CheckCircle2, XCircle, Circle, Loader2, Smartphone, Layout, ListCheck, 
  RefreshCcw, Eye, AlertTriangle, Lock, Cpu, ShieldCheck, ChevronRight, Zap,
  Home, Building2, Settings, Plus, Trash2, X, History, Save, FileText, 
  ChevronLeft, ExternalLink, ClipboardCheck, Tag, AlignLeft, FolderOpen, 
  FolderPlus, Layers, Users, PlusCircle, UserCircle, Sparkles, LogOut, Camera, User,
  Edit2, Check, Settings as SettingsIcon, AlertOctagon, ChevronUp, ChevronDown, Folder
} from 'lucide-react';

// --- 외부 독립 실행을 위한 표준 Firebase Imports ---
...
// --- Vercel 환경 변수 및 Firebase 설정 (컴파일 오류 방지 버전) ---
...
// --- 캔버스 종속 경로 제거: 독립된 파이어베이스의 직관적인 Root Collection 사용 ---
const getPublicCollection = (colName) => collection(db, colName);
const getPublicDoc = (colName, docId) => doc(db, colName, docId);
...
const App = () => {
  ...
  // --- Auth Listener (독립 외부 앱 배포용으로 완벽 수정됨) ---
  useEffect(() => {
  ...
  const analyzeScreenshot = async (base64Data, imageSrc) => {
    ...
    const callApi = async (attempt = 0) => {
      try {
        ...
        if (attempt < 5) { await new Promise(r => setTimeout(r, Math.pow(2, attempt) * 1000)); return callApi(attempt + 1); }
        throw err;
      }
    };
